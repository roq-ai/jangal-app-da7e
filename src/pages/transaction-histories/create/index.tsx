...
import { createTransactionHistory } from 'apiSdk/transaction-histories';
...
function TransactionHistoryCreatePage() {
  ...
  const handleSubmit = async (values: TransactionHistoryInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createTransactionHistory(values);
      resetForm();
  ...
  const formik = useFormik<TransactionHistoryInterface>({
    initialValues: {
      ...
      transaction_status: '',
      sender_account_number: (router.query.sender_account_number as string) ?? null,
      reciever_account_number: (router.query.reciever_account_number as string) ?? null,
    },
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Create Transaction History',
              isCurrent: true,
            },
          ]}
        />
