...
import { getTransactionHistories } from 'apiSdk/transaction-histories';
import { TransactionHistoryInterface } from 'interfaces/transaction-history';

import { TransactionHistoryListPage as ShowCaseList } from 'pages/transaction-histories';
...
function DashboardPage() {
  ...
  const transactionHistoryStat = useSWR<PaginatedInterface<TransactionHistoryInterface>>(
    () => `/transaction-histories`,
    () => getTransactionHistories({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: transactionHistoryStat?.data?.totalCount,
      isLoading: transactionHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`transaction-histories`} passHref legacyBehavior>
                  <Button
